...
const ArchiveItem: React.FC<{ type: string; title: string; date?: string; link: string }> = ({ type, title, date, link }) => {
  ...
  const content = (
    <div className="group block neo-brutalism-border bg-white p-4 hover:bg-black hover:text-white transition-all duration-300 ease-out">
      ...
      <h4 className="text-lg font-black uppercase leading-tight group-hover:translate-x-1 transition-transform duration-300 ease-out">
        {title}
      </h4>
    </div>
  );
